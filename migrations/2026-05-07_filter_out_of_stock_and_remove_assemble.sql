-- Migration : filtrer les produits en rupture par défaut + supprimer les "à composer"
-- Date : 2026-05-07
-- Contexte :
--   - 1363 produits sur 3812 sont en rupture (36%) — pollution massive des résultats
--   - 16 produits "à composer" (kits Ouray + Casafan Impérial) sont des moteurs/kits à
--     monter soi-même, pas des ventilateurs prêts à l'emploi → à exclure du moteur
--
...
-- ─── Phase 1 : nouvelle signature de la RPC avec p_include_out_of_stock ───

DROP FUNCTION IF EXISTS public.fan_boutique_search_v2(text, text[], text[], text[], text, boolean, boolean, boolean, boolean, boolean, boolean, integer, integer, integer, numeric, numeric, boolean, boolean, boolean, boolean, boolean, text[], boolean, boolean, boolean, boolean, integer, integer, numeric, numeric, integer, integer, integer, text[], text, text, integer, text);
...
CREATE OR REPLACE FUNCTION public.fan_boutique_search_v2(
    ...
    p_include_out_of_stock boolean DEFAULT FALSE  -- ⭐ NOUVEAU : exclu par défaut
)
RETURNS TABLE(
    id bigint, prestashop_id integer, nom text, reference text,
...
BEGIN
  ...
  WITH base AS (
    SELECT
      p.id, p.prestashop_id, p.nom, p.reference,
      p.prix_ttc, p.prix_promo, p.en_stock,
      p.image_url, p.product_url, p.description_courte,
      p.style, p.marque, p.gamme, p.couleur_moteur, p.couleur_pales,
      p.type_moteur, p.silencieux, p.diametre_cm, p.nombre_pales,
      p.avec_lumiere, p.avec_telecommande, p.wifi, p.reversible,
      p.option_destratificateur, p.garantie, p.score_reparabilite,
      p.type_produit, p.pieces, p.surface_max_m2,
      p.usage_exterieur, p.indice_protection, p.puissance_watts, p.classe_energetique,
      p.matiere_pales, p.commande_vocale, p.app_telephone, p.lumiere_dimmable,
      p.sonde_thermostatique, p.prolongateur_dispo, p.longueur_max_prolongateur,
      p.plafond_en_pente, p.boitier_mural_adaptable, p.distance_plafond_pales_cm,
      p.surface_min_m2, p.surface_destrat_m2, p.hauteur_max_destrat,
      p.pales_reversibles_bicolores, p.kit_lumiere_option, p.telecommande_adaptable,
      p.type_source_lumineuse, p.sous_type,
    ...
    WHERE
      -- ⭐ NOUVEAU : exclure les rupture par défaut
      (p_include_out_of_stock OR p.en_stock = true)
      AND (p_type_produit IS NULL OR p.type_produit = p_type_produit)
  ...
  SELECT
    b.id, b.prestashop_id, b.nom, b.reference,
...
-- ─── Phase 2 : supprimer les 16 produits "à composer" ───

DELETE FROM public.fan_boutique_products_v2
WHERE LOWER(nom) LIKE '%à composer%';

-- Vérification
SELECT COUNT(*) AS produits_restants FROM public.fan_boutique_products_v2;
